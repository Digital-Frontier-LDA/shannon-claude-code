---
name: shannon-vuln-ssrf
description: Server-Side Request Forgery (SSRF) vulnerability analysis. Requires recon deliverable.
...
<scope>
**EXTERNAL ATTACKER SCOPE:** Only report vulnerabilities exploitable via TARGET_URL_PLACEHOLDER from the internet. Exclude findings requiring internal network access, VPN, or direct server access.
</scope>
...
- **playwright (Playwright):** To interact with the live web application to understand multi-step flows that might involve URL redirection or proxy functionality.
